...
from ..specs import DroneRuntime, ExecutionLayout, LogicalRing, ProtectionPlan, RuntimeState, SystemSpec
from ..protection_state import owner_protection_view, update_protection_runtime_at_completed_token
from ..experiments.scenarios import make_initial_layout, make_initial_ring, make_standard_system
from .traces import SummaryRow, TokenTraceRow, UAVTraceRow
...
from ..specs import DroneRuntime, ProtectionPlan, ProtectionRuntime, RuntimeState, SystemSpec
...
def simulate_fixed_plan_with_recovery(
    ...
    *,
    ...
    token_trace = [_make_token_row_recovery(run_id, plan.method, system, state, initial_latency, failures_so_far)]
    stage0 = _dynamic_stage_latency_by_uav(system, plan, _alive_set(state), recovered_exec_layers_by_uav)
    uav_trace = _make_uav_rows_recovery(run_id, plan.method, system, plan, state, stage0, recovered_exec_layers_by_uav)
    ...
    for token in range(1, max_tokens + 1):
        ...
        token_trace.append(_make_token_row_recovery(run_id, plan.method, system, state, next_latency, failures_so_far))
        uav_trace.extend(_make_uav_rows_recovery(run_id, plan.method, system, plan, state, stage, recovered_exec_layers_by_uav))
